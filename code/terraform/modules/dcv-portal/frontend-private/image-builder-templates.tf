...
module "image_builder_templates" {
  ...
  config = {
    ...
    asg_load_balancer_arn   = var.module_dcv.load_balancer.arn
    asg_vpc_id              = var.module_network.vpc.id

    image_builder_instance_profile        = var.module_dcv.image_builder_instance_profile.name
    image_builder_pipeline_logs           = var.module_dcv.image_builder_pipeline_logs.id
    ...
    dcv_lambda_update_tags_function_name  = var.module_dcv.dcv_lambdas["image-builder-update-tags"].lambda.function_name
    dcv_lambda_run_pipeline_function_name = var.module_dcv.dcv_lambdas["image-builder-run-pipeline"].lambda.function_name
  }
}
