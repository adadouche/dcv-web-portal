...
def lambda_handler(event, context):
    ...
    for reservation in describe_response['Reservations']:
        for instance in reservation['Instances']:
            tag_cognito_groups = ''.join(
                [tag['Value'] for tag in instance['Tags'] if tag['Key'] == 'cognito_groups'])
    ...
    if len(instanceIds) > 0:
        ...
        # logger.debug(terminate_response)

    response_body = terminate_response['TerminatingInstances']
