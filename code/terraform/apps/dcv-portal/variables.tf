...
variable "created_by_email" {
  description = "Please provide your email address: (it will be used to tag resources and to create the Portal Administrator user if you don't provide one)"
  type        = string
  validation {
    condition = length(var.created_by_email) >= 6 && can(regex("^[\\w-_+\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$", var.created_by_email))
...
variable "authentication_config" {
  type        = object({
    ...
    use_cognito_proxy = optional(bool)
  })
  default     = {
...
variable "network_config" {
  ...
  default = {
    ...
    ip_allow_list         = null # set it to null so that the current machine ip is used in the ip allow list
